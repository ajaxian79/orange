...
namespace Orange::UI {
  ...
  DrawButtonImage(const std::shared_ptr<Orange::Image> &imageNormal, const std::shared_ptr<Orange::Image> &imageHovered,
                  ...
                  ImU32 tintNormal, ImU32 tintHovered, ImU32 tintPressed,
                  ImRect rectangle) {
    return DrawButtonImage(imageNormal, imageHovered, imagePressed, tintNormal, tintHovered, tintPressed, rectangle.Min,
                           rectangle.Max);
  };
  ...
  DrawButtonImage(const std::shared_ptr<Orange::Image> &imageNormal, const std::shared_ptr<Orange::Image> &imageHovered,
                  const std::shared_ptr<Orange::Image> &imagePressed,
                  ImU32 tintNormal, ImU32 tintHovered, ImU32 tintPressed) {
    return DrawButtonImage(imageNormal, imageHovered, imagePressed, tintNormal, tintHovered, tintPressed,
                           ImGui::GetItemRectMin(), ImGui::GetItemRectMax());
  };

  bool DrawButtonImage(const std::shared_ptr<Orange::Image> &image,
                       ImU32 tintNormal, ImU32 tintHovered, ImU32 tintPressed) {
    return DrawButtonImage(image, image, image, tintNormal, tintHovered, tintPressed, ImGui::GetItemRectMin(),
    ...
    auto CalcWindowAutoFitSize = [CalcWindowSizeAfterConstraint](ImGuiWindow *window, const ImVec2 &size_contents) {
      ...
      if (window->Flags & ImGuiWindowFlags_Tooltip) {
        ...
        ImVec2 size_auto_fit = ImClamp(size_desired, size_min,
                                       ImMax(size_min, {avail_size.x - style.DisplaySafeAreaPadding.x * 2.0f,
    ...
    for (int resize_grip_n = 0; resize_grip_n < resize_grip_count; resize_grip_n++) {
      ...
      if (held && g.IO.MouseDoubleClicked[0] && resize_grip_n == 0) {
        ...
        const float x = g.IO.MousePos.x - g.ActiveIdClickOffset.x +
                        ImLerp(def.InnerDir.x * grip_hover_outer_size, def.InnerDir.x * -grip_hover_inner_size,
                               def.CornerPosN.x);
        const float y = g.IO.MousePos.y - g.ActiveIdClickOffset.y +
                        ImLerp(def.InnerDir.y * grip_hover_outer_size, def.InnerDir.y * -grip_hover_inner_size,
                               def.CornerPosN.y);
